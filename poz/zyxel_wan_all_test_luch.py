...
def speed_zyxel(host,port,delay):
	...
	try:	
            ...
            #LUCH_0_199 service na zyxel port 7654
			telnet.write("show conn service LUCH_0_199"+" \n")
			...
			if wanr[0] == "B":
				wanrx = float(wanrx)
			if wanr[0] == "K":
				wanrx = float(wanrx*1024)
			if wanr[0] == "M":
				wanrx = float(wanrx*1048576.0)
			if wanr[0] == "G":
				wanrx = float(wanrx*1073741824.0)
			...
			if want[0] == "B":
				wantx = float(wantx)
			if want[0] == "K":
				wantx = float(wantx*1024.0)
			if want[0] == "M":
				wantx = float(wantx*1048576.0)
				print want[0]
			if want[0] == "G":
				wantx = float(wantx*1073741824.0)
...
c = 5	
r,t=speed_zyxel("192.168.146.225","5188",int(c))
print r,t
