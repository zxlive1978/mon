...
function init(){
	...
	*/
	...
	line.stroke({ width: 5, color: '#e0e0e0', dasharray: '5,5' });
	...
	*/
	...
	var weight_colmn1 = (100-Number(Sheet.time_w))/Number(Sheet.numbs_colmns);
	//Высота одного параметра box
	var height_colmn1_p1 = h1*Number(Sheet.disp_up) / Number(Sheet.numbs_colmn1);
	
	//Шапка Угловой квадрат
	var colmn0 = draw.polygon('0, 0 '+w1*Number(Sheet.time_w)+',0 ' + w1*Number(Sheet.time_w) + ',' + h1*Number(Sheet.disp_up) + ' 0,' + h1*Number(Sheet.disp_up))
	.fill({ color: '#e0e0e0' })
	.stroke({ width: 2 });
	...
	*/
	...
	for (var j=0; j< comment.length;j++){
			var ncolcmt = colmn1_x0;
			var left = comment[j]["left_txt"];
			if (left>50){ncolcmt =colmn21_x0;}
			if (left>100){ncolcmt = colmn31_x0;}
			if (left>140){ncolcmt = colmn41_x0;}
			...
			//console.log(name_p3);
			var cur_comm =""
			for (var i=0; i< name_p3.length;i++){ 
				name_p3[i]=name_p3[i] + '\n'
				cur_comm = cur_comm + name_p3[i]
			}
			var text_name_p3 = draw.text(cur_comm)
				.font({ family: 'Inconsolata', size: size_text_p  })
				.move(ncolcmt+w1*weight_colmn1/2, cury)
				.center(ncolcmt+w1*weight_colmn1/2, cury)
				.fill(color44)
	...
	//Поиск дырки
	var hole ="";
	if (d110d[0]["Vrema"]-start_time>120){
			hole+=j+", "+(j+1)+", ";
		}
	if (end_time-d110d[d110d.length-1]["Vrema"]>120){
			hole+=j+", "+(j+1)+", ";
		}
	for (var j=0; j< d110d.length-1;j++){
	...
	for (var j=0; j< (holearr.length)-1;j+=2){
		var holl1=Number(holearr[j]);
		var holl2=Number(holearr[j+1]);
		//console.log(holl1,holl2);
		...
		.fill({ color: '#ff0066' });
		hol1.attr({'fill-opacity': 0.2});
	
	}
	/*
	//for (var i=0; i< holearr.length-1;i++){
		for (var j=0; j< holearr.length-1;j++){
			if (holearr[j+1]-holearr[j]==1 && holearr[j+2]-holearr[j+1]==1){
				holearr.splice(j+1, );
			}
		//}
	}
	*/
	...
	gfx1.click(function() {
		...
		var day = new Date(d110d[disp_val1]["Vrema"]*1000);
		//alert (day);
		var last_hour = day.getHours();
		var minutes = day.getMinutes();
		if (last_hour<10){last_hour="0"+last_hour;}
		...
		var month = day.getMonth() + 1;
		if (month<10){month="0"+month;}
		var time_viz = ' '+last_hour+":"+minutes+" "+dates+"-"+month +(2000-yearr);
		var text_value_16 = draw.text(time_viz)
		.font({ family: 'Inconsolata', size: size_text_p +3})
